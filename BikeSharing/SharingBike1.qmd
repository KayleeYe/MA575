...
```{r}
datasetBike <- read.csv("D:/copied from desktop/Elsa BUBUBU/大三下/ma575/project/lab2/day.csv")
head(datasetBike)
summary(datasetBike)
```
...
modela <- lm(cnt ~ temp, data=datasetBike)
summary(modela)
residuals <- residuals(modela)
plot(temp, residuals, xlab="Normalized Temperature", ylab="Residuals", main="Residual Plot")
abline(h=0, col="red")
#2
scatterplot(wind,cnt)
modelb <- lm(cnt ~ wind, data=datasetBike)
summary(modelb)
residuals <- residuals(modelb)
plot(wind, residuals, xlab="Wind", ylab="Residuals", main="Residual Plot")
abline(h=0, col="red")
#3
scatterplot(hum,cnt)
modelc <- lm(cnt ~ hum, data=datasetBike)
summary(modelc)
residuals <- residuals(modelc)
plot(hum, residuals, xlab="Humidity", ylab="Residuals", main="Residual Plot")
abline(h=0, col="red")
